...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c => c).map{case (c,s)=>(c,s.size)}.toList.sortBy(x => x)
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.map(xn=>y.find(_._1==xn._1) match{
    case Some(yn)=>(xn._1,xn._2-yn._2)
    case None =>xn
    ...
    def rec(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ =>
        for {
          ...
          tail <- rec(subtract(occurrences, occurrence))
        } yield word :: tail
    }
