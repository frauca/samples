...
object LineOfSight {
  ...
  def angle(input:Array[Float],idx: Integer):Float = if(idx==0) 0 else input(idx) / idx

  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    @tailrec
    def lineOfSightRec(idx:Int,maxAngle:Float): Unit ={
      if(idx<input.length){
        val newAngle:Float=max(angle(input,idx),maxAngle)
        output(idx)=newAngle
        lineOfSightRec(idx+1,newAngle)
      }
    }
    lineOfSightRec(0,0);
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var res=0;
    //println(s"upsweepSequential $from $until")
    if(from == until){
      throw new Error("Same value")
    }
    (from until until).map(i => angle(input,i)).reduce(_ max _)
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    if(from<until){
      val newAngle=max(angle(input,from),startingAngle)
