...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if(size>minimumSize){
        ...
        (bodies:+b).foldLeft(fork){(fork,b)=>fork.insert(b)}
      }else{
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      val sector = normalizeY(boundY(b.y)).toInt * sectorPrecision + normalizeX(boundX(b.x)).toInt
