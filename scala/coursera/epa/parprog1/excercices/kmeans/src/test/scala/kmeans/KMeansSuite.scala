...
class KMeansSuite extends FunSuite {
  ...
  def checkVector(result: GenSeq[Point], expected: GenSeq[Point]): Unit ={
    assert(result.zip(expected).forall{case(x,y)=>x.toString==y.toString},
      s"vector $result should equal to $expected")
  }
  ...
  test("'kMeans' should work for 'points' == GenSeq((0, 0, 1), (0,0, -1), (0,1,0), (0,10,0)) and 'oldMeans' == GenSeq((0, -1, 0), (0, 2, 0)) and 'eta' == 12.25") {
    ...
    checkVector(kMeans(points,means,12.5), expected)
  }
