...
@pytest.mark.asyncio
async def test_get_valid_user(client: AsyncClient, user: User) -> None:
    new_user = User(**user.model_dump())
    new_user.id = None
    new_user.name = "get valid test user"
    new_user.email = "other@fake.mail"
...
async def test_user_not_found(client: AsyncClient) -> None:
    ...
    assert (
        response.json()["detail"] == "The user with the id '666666' could not be found"
    )
...
async def test_duplicated_email(client: AsyncClient, user: User) -> None:
    ...
    assert (
        ...
        == f"The email is already present in our system. To update the user you need to know the id of the user."
    )
