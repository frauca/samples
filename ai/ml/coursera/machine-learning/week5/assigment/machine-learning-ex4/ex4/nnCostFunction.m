...
y_r = eye(num_labels)(y,:)'; %y relabeled to vecotrs
J_t = y_r .* log(A3) + (1-y_r) .* log(1-A3);
O_L1=Theta1(:,2:end);
O_L2=Theta2(:,2:end);
J = (-1/m)*sum(sum(J_t)) + (lambda/(2*m)) * sum([(O_L1.^2)(:);(O_L2.^2)(:)]);
...
%for i = 1:m
  ...
  %A1_t  = A1(i,:)';
  %delta3_t = delta3(:,i);
  %delta2 = (Theta2'*delta3_t) .* sigmoidGradient(Z2_t);
  
  %Theta2_grad = Theta2_grad + delta3_t * A2_t';
  %Theta1_grad = Theta1_grad + delta2(2:end) * A1_t';
%endfor
  Theta1_grad = (1/m)*Theta1_grad;
  Theta2_grad = (1/m)*Theta2_grad;
%printf('Total cpu time for bckground : %f seconds\n', cputime-t);
% -------------------------------------------------------------
